...
st.markdown("""
    ...
    .stat-box { background: #f8f9fa; border: 1px solid #dee2e6; padding: 10px; border-radius: 10px; text-align: center; }
    </style>
    """, unsafe_allow_html=True)

# --- 2. LOGIQUE EXPERTE & STATISTIQUES ---
...
def obtenir_stats_ia():
    try:
        df = pd.read_csv(DB_FILE)
        if df.empty: return None
        stats = df.groupby('discipline')['resultat'].agg(['mean', 'count']).reset_index()
        stats.columns = ['Discipline', 'Réussite', 'Courses']
        stats['Réussite'] = (stats['Réussite'] * 100).round(1).astype(str) + "%"
        return stats
    except: return None
...
st.title("🧠 TurfMaster AI Pro v6.0")

# --- Onglets pour séparer Analyse et Apprentissage ---
tab1, tab2 = st.tabs(["🚀 Analyse Course", "📊 Mémoire IA"])

with tab1:
    col1, col2 = st.columns(2)
    with col1:
        capital = st.number_input("💰 Capital (€)", value=500, key="cap")
    ...
    st.markdown(f"""<div class="ia-status">🤖 <b>Stratégie :</b> {discipline} | <b>Confiance IA :</b> {adv_dynamique}</div>""", unsafe_allow_html=True)
    ...
    if 'df_course' in st.session_state and not st.session_state.df_course.empty:
        ...
        st.markdown(f"""<div class="pronostic-box"><b>🏁 ORDRE ESTIMÉ :</b><br>
        ...
        st.markdown("---")
        arrivee_txt = st.text_input("🏁 2. Collez l'arrivée pour apprendre :")
        if st.button("💾 ENREGISTRER L'ARRIVÉE"):
            if arrivee_txt:
                ...
                st.success("Apprentissage validé !")
                st.rerun()

with tab2:
    st.header("📈 Performances de l'IA")
    stats_df = obtenir_stats_ia()
    if stats_df is not None:
        st.table(stats_df)
        if st.checkbox("Voir l'historique complet"):
            st.dataframe(pd.read_csv(DB_FILE).tail(50))
        if st.button("🗑️ Réinitialiser la mémoire"):
            os.remove(DB_FILE)
            initialiser_ia()
            st.rerun()
    else:
        st.info("Aucune donnée d'apprentissage pour le moment. Enregistrez des arrivées !")
