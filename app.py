...
def calculer_analyse_ia(cote, capital, discipline, fraction=0.25):
    ...
    # Estimation de l'ordre d'arrivée basée sur la force de la Value
...
def extraire_texte(texte):
    # Regex améliorée pour capturer Numéro, Nom et Cote même si mélangés
    # Cherche un chiffre (numéro), puis du texte (nom), puis un chiffre avec virgule (cote)
    pattern = r"(\d{1,2})\s+([A-ZÀ-Z\s]{3,})\s+.*?(\d+[\.,]\d+)"
...
with tab2:
    txt_in = st.text_area("Colle le texte ici :", height=150)
...
if not df_final.empty:
    ...
    for _, row in df_final.iterrows():
        ...
        # FILTRE STRICT : Uniquement les probabilités > 60%
        if podium >= 60:
            res = row.to_dict()
    ...
    if resultats:
        st.success(f"Détection de {len(resultats)} chevaux à haute probabilité (>60%)")
        for res in resultats:
            ...
            <div class="card">
                ...
                <div class="prediction-line">🎯 Prévision : {res['prev']}</div>
            </div>
            """, unsafe_allow_html=True)
    else:
        st.warning("Aucun cheval ne dépasse les 60% de probabilité de podium sur cette course.")
