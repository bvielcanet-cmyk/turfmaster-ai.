...
# --- 1. CONFIGURATION ---
st.set_page_config(page_title="TurfMaster 2.5 : Ticket Express", page_icon="🚀", layout="wide")
...
def simulation_performance_max(df, hippo, discipline, capital, profil):
    ...
    PARTANTS (Numéros Réels) : 
    {partants_str}

    MISSION :
    1. Détermine Météo/Distance pour {hippo}.
    2. Calcule le TOP 5 Probabilités. 
    3. IMPORTANT : Détecte un Outsider (🔥) avec un potentiel de gain élevé et intègre-le dans le calcul des probas.
...
# --- 3. INTERFACE ---
st.title("🚀 TurfMaster 2.5 : Ticket & Outsider")
        ...
        with st.spinner("🧪 Analyse des cotes et détection d'outsider..."):
            ...
            if raw:
                try:
                    ...
                    # 3. TICKET FINAL (Numéros à jouer inclus)
                    st.divider()
                ...
                except: st.error("Erreur d'analyse. Vérifiez le format de saisie.")
    else: st.error("Veuillez saisir des partants (ex: 12 NOM).")

st.divider()
st.caption("Numéros à jouer intégrés | Détection Outsider Active | Gemini 2.5 Flash")
