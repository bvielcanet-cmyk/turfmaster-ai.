...
def extraire_partants_robuste(texte):
    ...
    if current_horse: partants.append(current_horse)
    for p in partants:
        if p["cote"] is None or p["cote"] <= 1.0: p["cote"] = 20.0 # Sécurité si cote non trouvée
...
def expertise_live_gemini(discipline, hippodrome, df_partants):
    ...
    2. Chevaux chauds (Déferrage D4, jockeys en forme).
    3. Identifie 1 OUTSIDER (cote > 15) capable de surprendre.
    ...
    except: return "⚠️ Expertise Web indisponible."
...
with tab1:
    ...
    if st.button("🚀 LANCER L'ANALYSE"):
        ...
        if not df_course.empty:
            ...
            # --- CALCULS STRATÉGIQUES SÉCURISÉS ---
            resultats = []
            best_value = -1
            pari_maitre = None

            for _, row in df_course.iterrows():
                # PROTECTION ANTI-CRASH : Cote minimum 1.01
                cote_safe = max(float(row['cote']), 1.01)
                
                prob_ia = (1 / cote_safe) * 1.15
                podium = min(98, int((prob_ia ** 0.7) * 100 * 2.2))
                
                # Formule de Kelly sécurisée
                denom = cote_safe - 1
                f_kelly = (prob_ia * denom - (1 - prob_ia)) / denom
                
                mise = min(max(0, capital * f_kelly * 0.15), capital * 0.10)
                value_index = (prob_ia * cote_safe) - 1
                
                res_obj = {**row, "cote": cote_safe, "podium": podium, "mise": mise, "value": value_index}
            ...
            if opportunite:
                ...
                <small>Risque trop élevé ou value négative. L'IA conseille de passer votre tour.</small></div>""", unsafe_allow_html=True)

            # Cartes détaillées
            cols = st.columns(3)
            res_sorted = sorted(resultats, key=lambda x: x['podium'], reverse=True)
