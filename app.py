...
# Style CSS Pro (Texte Noir & Lisibilité)
st.markdown("""
    <style>
...
st.title("🏇 TurfMaster AI Cloud Pro")
df_ia = load_data()
...
with tab1:
    ...
    st.markdown(f"""<div class="ia-status">🤖 <b>Mémoire Cloud Active</b> | Avantage : <b>{adv_dynamique}</b></div>""", unsafe_allow_html=True)
    ...
    if 'df_course' in st.session_state and not st.session_state.df_course.empty:
        ...
        st.session_state.derniers_resultats = resultats # Pour sauvegarde
...
with tab2:
    st.header("📊 Historique de l'IA (Google Sheets)")
    if not df_ia.empty:
        ...
        st.info("Le Google Sheet est vide. Enregistrez une arrivée pour commencer.")
