...
# --- 1. CONFIGURATION ---
st.set_page_config(page_title="TurfMaster AI Pro Diagnostic", page_icon="🛡️", layout="wide")
...
st.markdown("""
    ...
    .error-box { background-color: #fce4ec; border-left: 5px solid #d81b60; padding: 10px; border-radius: 8px; color: #880e4f; font-size: 12px; margin-bottom: 20px; }
...
def expertise_live_gemini(discipline, hippodrome, df_partants):
    ...
    3. Identifie 1 OUTSIDER (cote > 15).
    4. Propose un TRIO.
    """
    try:
        ...
        # On retourne l'erreur brute pour le diagnostic
        return None, str(e)
    ...
    if st.button("🚀 LANCER L'ANALYSE"):
        ...
        if not df_course.empty:
            with st.spinner(f"🌐 Gemini explore le web pour {hippo}..."):
                expertise, erreur_technique = expertise_live_gemini(discipline, hippo, df_course[['num', 'nom']])
            ...
            if expertise:
                ...
                st.markdown(f"""
                <div class="error-box">
                    ⚠️ <b>Expertise Web indisponible (Mode Statistique Activé)</b><br>
                    Erreur détectée : <i>{erreur_technique}</i><br><br>
                    <small>Vérifiez vos quotas Google AI Studio ou votre clé API.</small>
                </div>
                """, unsafe_allow_html=True)
            ...
            if opportunite:
                ...
                st.markdown("""<div class="no-bet">⚠️ AUCUN PARI RENTABLE DÉTECTÉ</div>""", unsafe_allow_html=True)
