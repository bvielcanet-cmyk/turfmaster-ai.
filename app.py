...
# --- 1. CONFIGURATION ---
st.set_page_config(page_title="TurfMaster AI Pro", page_icon="🛡️", layout="wide")
tz_paris = pytz.timezone('Europe/Paris')

# Sécurité Clé API Gemini
...
st.markdown("""
    ...
    .num-badge { background-color: #34495e; color: white; padding: 2px 10px; border-radius: 5px; font-weight: bold; margin-right: 8px; font-size: 18px; }
...
def extraire_partants_robuste(texte):
    partants = []
    # Nettoyage des espaces insécables et tabulations qui bloquent l'extraction
    ...
    for i, ligne in enumerate(lignes):
        ...
        # Détection du NOM (Ligne en MAJUSCULES sans chiffres)
        if ligne.isupper() and len(ligne) > 3 and not any(c.isdigit() for c in ligne):
            if current_horse: partants.append(current_horse)
            
            # Recherche du numéro (ligne précédente ou compteur)
            num_final = str(compteur_auto)
            if i > 0 and lignes[i-1].strip().isdigit():
                num_final = lignes[i-1].strip()
            
            current_horse = {"num": num_final, "nom": ligne, "cote": None, "musique": "Inconnue"}
            compteur_auto += 1

        # Détection de la MUSIQUE
        elif re.search(r'\d+[apmsh]', ligne.lower()) and current_horse:
            current_horse["musique"] = ligne

        # Détection de la COTE (Cherche un nombre décimal type 12.5 ou 1,4)
        else:
    ...
    # Sécurité : Si aucune cote n'est trouvée, on évite le calcul à 0
    for p in partants:
        if p["cote"] is None or p["cote"] == 0: p["cote"] = 20.0
        
    return pd.DataFrame(partants)

def expertise_live_gemini(discipline, hippodrome, df_partants):
    prompt = f"""
    Effectue une recherche Google en temps réel sur la course de {discipline} à {hippodrome} AUJOURD'HUI.
    Partants : {df_partants.to_string()}
    1. État du terrain et météo sur place.
    2. Chevaux "chauds" (bruits d'écurie, déferrage D4, jockeys en forme).
    3. Identifie 1 OUTSIDER (cote > 15) capable de finir dans les 3.
    ...
    except: return "⚠️ Expertise Web indisponible. Analyse statistique activée."
...
tab1, tab2 = st.tabs(["🚀 Analyse & Pari Optimal", "📊 Mémoire Cloud"])
    ...
    if st.button("🚀 LANCER L'ANALYSE"):
        ...
        if not df_course.empty:
            ...
            for _, row in df_course.iterrows():
                # On utilise un avantage IA fixe (ou dynamique selon ton historique)
            ...
            # Affichage du Filtre de Sécurité
            opportunite = (pari_maitre and best_value > 0.05 and 
            ...
            if opportunite:
                ...
                <div class="master-bet">
                    ...
                    <div style="font-size: 13px; opacity: 0.9;">Value : {round(best_value, 2)} | Confiance : {pari_maitre['podium']}%</div>
                </div>
                """, unsafe_allow_html=True)
            else:
                st.markdown("""<div class="no-bet">⚠️ AUCUN PARI RENTABLE DÉTECTÉ<br>
                <small>Risque trop élevé ou value négative. Passez votre tour.</small></div>""", unsafe_allow_html=True)
            ...
            st.markdown("---")
            arrivee = st.text_input("🏁 Résultat final (ex: 4-1-10) :")
            if st.button("💾 SYNCHRONISER AU CLOUD"):
                st.success("✅ Données prêtes à l'archivage.")
        else:
            st.error("Aucun cheval détecté. Vérifiez le format (Noms en MAJUSCULES).")

with tab2:
    st.header("📊 Historique Cloud")
    if not df_ia.empty: st.dataframe(df_ia.tail(15))
    else: st.info("Le Google Sheet est vide.")
